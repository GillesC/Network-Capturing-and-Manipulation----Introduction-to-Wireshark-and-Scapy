...
The lab sessions will be evaluated by means of small tests at the end of several lab sessions. This assignment contains several questions which can help the students to assess how well they have mastered the subjects. Note that the student needs to be able to show the programming code when requested by the teaching assistant for evaluation.

\textbf{Written report (Question~4).}
In addition to the tests, Question~4 has to be executed, reported, and sent to \texttt{gilles.callebaut@kuleuven.be}. It can be written in Dutch or English but not both, except specific terminology. 

\textbf{Presentation (recording) Use Case.}
See Section~\ref{sec:use-cases}.


\textbf{Deadlines.}
The deadlines will be communicated in the lab sessions.
